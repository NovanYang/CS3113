    ...
    if (!g_pause) {
        if (!g_game_end) {
            ...
            if (g_current_scene == g_levelA && g_current_scene->m_state.player->get_position().y < -10.0f) {
                if (g_levelA && g_current_scene->m_state.player->get_position().x < 12.0f) {
                    if (lives > 1) {
                        switch_to_scene(g_current_scene);
                        lives -= 1;
                    }
            ...
            if (g_current_scene == g_levelB && g_current_scene->m_state.player->get_position().y < -10.0f) {
                if (g_levelB && g_current_scene->m_state.player->get_position().x < 12.0f) {
                    if (lives > 1) {
                        switch_to_scene(g_current_scene);
                        lives -= 1;
                    }
            ...
            if (g_current_scene == g_levelC && g_current_scene->m_state.player->get_position().y < -10.0f) {
                if (g_levelC && g_current_scene->m_state.player->get_position().x < 12.0f) {
                    if (lives > 1) {
                        switch_to_scene(g_current_scene);
                        lives -= 1;
                    }
...
void render()
{
    ...
    if (g_current_scene != g_menu) {
        if (!g_game_end) {
            Utility::draw_text(&g_shader_program, font_texture_id, "Lives Remaining: " + std::to_string(lives), 0.3, -0.1, glm::vec3(1.0f, -2.0f, 0.0f));
        }
        else {
            if (g_game_win) {
                Utility::draw_text(&g_shader_program, font_texture_id, win, 0.3, -0.1, glm::vec3(14.0f, -2.0f, 0.0f));
            }
            else if (g_game_lose) {
                Utility::draw_text(&g_shader_program, font_texture_id, lose, 0.3, -0.1, glm::vec3(12.0f, -2.0f, 0.0f));
            }
        }
    }
    glUseProgram(g_shader_program.get_program_id());
